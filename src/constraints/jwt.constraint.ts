...
function checkExistFolder(folderName: string) {
	const folderPath = path.join(__dirname, `../../${folderName}`);
	fs.mkdirSync(folderPath, { recursive: true }); // Đảm bảo thư mục tồn tại
}
...
function getAccessTokenKeyPair() {
	...
	generateKeyPairIfNeeded(privateKeyPath, publicKeyPath);

	return {
		access_token_private_key: fs.readFileSync(privateKeyPath, 'utf-8'),
		access_token_public_key: fs.readFileSync(publicKeyPath, 'utf-8'),
	};
}
...
function getRefreshTokenKeyPair() {
	...
	return {
		refresh_token_private_key: fs.readFileSync(privateKeyPath, 'utf-8'),
		refresh_token_public_key: fs.readFileSync(publicKeyPath, 'utf-8'),
	};
}
